...
const PlinkoBoard: React.FC<PlinkoBoardProps> = ({ 
  ...
  const WIDTH = minContentWidth + 20; // Even tighter margins (was +40)

  const START_Y = 60; // Moved UP significantly (was 100) to maximize vertical space
  const START_X = WIDTH / 2;
  const ROW_SPACING = 45; 
  const PEG_RADIUS = 5; 
  const BALL_RADIUS = 8; 
  
  // Visual Bucket Config - BIGGER BOXES
  const VISUAL_BUCKET_WIDTH = 48; 
  const VISUAL_BUCKET_GAP = 2;
  const VISUAL_BUCKET_HEIGHT = 60; // Increased from 40 to 60 for better visibility
    ...
    const handleResize = () => {
      if (containerRef.current) {
        ...
        if (parent) {
           ...
           // Padding reduced to 0px on mobile logic inside component essentially
           ...
           // Allow scaling up to 2x on large screens if there's space
  ...
  useEffect(() => {
    ...
    // Gravity: 1.2 allows for good acceleration as it falls
      ...
      event.pairs.forEach((pair) => {
        ...
        if (sensor && ball) {
           ...
           if (!ball.isStatic && !(ball as any).isProcessed) { 
             ...
             setActiveBucket({ index, ts: Date.now() });
  ...
  useEffect(() => {
    ...
    if (renderRef.current) {
        ...
        renderRef.current.canvas.height = boardHeight;
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < pinsInRow; col++) {
        ...
        const pin = Matter.Bodies.circle(x, rowY, PEG_RADIUS, {
          ...
          restitution: 0.4, // REDUCED from 0.5 to make balls bounce less -> stick to center
          friction: 0
        });
        bodies.push(pin);
      }
  ...
  useEffect(() => {
    if (ballDropData && engineRef.current) {
      ...
      // Spawn ball slightly above the visual hole so it shoots THROUGH it
      const spawnY = START_Y - 50; 
      
      const ball = Matter.Bodies.circle(START_X + randomOffset, spawnY, BALL_RADIUS, {
        restitution: 0.6, // REDUCED from 0.8 to make ball "heavier" and less bouncy
        friction: 0.002,
        // Decreased frictionAir to allow acceleration as it falls (was 0.04)
        frictionAir: 0.015, 
        label: 'ball',
      ...
      // Very low initial velocity so it starts "slow" then speeds up due to gravity
  ...
  }, [ballDropData, START_X]); // Added START_X
  ...
  return (
    <div ref={containerRef} className="w-full h-full flex items-center justify-center overflow-hidden py-1 sm:py-4">
        ...
        >
            {/* Drop Hole Visual */}
            <div 
                className={`absolute z-20 rounded-full bg-[#0a0e17] border-4 flex items-center justify-center transition-all duration-100
                    ${isDropping ? 'border-lime-400 scale-110 shadow-[0_0_30px_rgba(132,204,22,0.6)]' : 'border-slate-700 shadow-xl'}
                `}
                style={{
                    top: `${START_Y - 50}px`, // Position hole above the start line
            ...
            >
                 <div className="absolute inset-0 rounded-full bg-black opacity-50 shadow-inner"></div>
                 {/* Flash Effect */}
                 {isDropping && (
