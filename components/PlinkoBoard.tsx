...
const PlinkoBoard: React.FC<PlinkoBoardProps> = ({ 
  ...
  const WIDTH = 1000;
  const START_Y = 100; // Moved down significantly to allow space for the hole above pegs
  const START_X = WIDTH / 2;
  const ROW_SPACING = 45; 
  const PEG_SPACING = 50; 
  const PEG_RADIUS = 5; 
  const BALL_RADIUS = 8; 
  // Visual Bucket Config
  const VISUAL_BUCKET_WIDTH = 48; 
  const VISUAL_BUCKET_GAP = 2;
  const VISUAL_BUCKET_HEIGHT = 40; 
    ...
    const handleResize = () => {
      if (containerRef.current) {
        ...
        if (parent) {
           ...
           const scaleX = (availWidth - 16) / WIDTH; // 16px padding
           const scaleY = (availHeight - 16) / boardHeight; 
           
           // Choose the smaller scale to ensure it fits
           const newScale = Math.min(scaleX, scaleY);
           
           // Limit max scale slightly less aggressively to allow filling large screens
                    ...
                    {multipliers.map((m, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    <span className="text-[14px] font-black text-[#1a0f05] leading-none drop-shadow-sm select-none">{m.value}x</span>
                                </div>
                            </div>
                        );
                    })}
