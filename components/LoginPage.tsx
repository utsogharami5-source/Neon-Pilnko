...
const LoginPage: React.FC = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Don't log full error to console to avoid user confusion, just handle the UI message
      let msg = "Authentication failed. Please try again.";
      
      if (err.code === 'auth/invalid-credential' || err.message?.includes('auth/invalid-credential')) {
          msg = isLogin 
            ? "Account not found or incorrect password. Did you mean to Sign Up?" 
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-[#0f172a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm sm:max-w-md relative z-10">
        <div className="bg-[#1e2535]/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-6 sm:p-8 shadow-2xl">
          ...
          <div className="mt-6 text-center">
            <button 
              ...
              className="text-slate-400 hover:text-white text-xs font-bold transition-colors py-2"
            >
