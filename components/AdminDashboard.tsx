...
import { WalletTransaction, TransactionStatus, PaymentMethodConfig, SystemAlert } from '../types';
import { CheckCircle, XCircle, Clock, Users, Wallet, Trash2, Archive, List, ArrowUpDown, ArrowUp, ArrowDown, Edit2, ChevronDown, PlusCircle, MinusCircle, Save, Lock, Unlock, Settings, ToggleLeft, ToggleRight, Smartphone, Banknote, AlertTriangle, Bell, Send, Megaphone, Search, Filter, Loader2 } from 'lucide-react';
        ...
        <div className="bg-[#0f1522] p-4 rounded-xl border border-slate-700 flex flex-col gap-3">
            <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2">
                        ...
                        >
                            {loadingId === method.id ? <Loader2 className="w-6 h-6 animate-spin" /> : (method.isEnabled ? <ToggleRight className="w-6 h-6" /> : <ToggleLeft className="w-6 h-6" />)}
                        </button>
                        <button 
...
const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'pending' | 'history' | 'users' | 'settings' | 'alerts' | 'all'>('pending');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        const a: SystemAlert[] = [];
        snapshot.forEach((doc) => {
            a.push({ id: doc.id, ...doc.data() } as SystemAlert);
        });
        setAlerts(a);
    }, (error) => {
  ...
  const handleStatusChange = async (newStatus: TransactionStatus, tx: WalletTransaction) => {
    ...
    // Only ask for confirmation/reason if it's a destructive action
    if (newStatus === 'rejected' || newStatus === 'cancelled') {
        const input = window.prompt(`Reason for ${newStatus} (Optional):`, "Admin Cancelled");
    ...
    try {
        ...
        setTransactions(prev => prev.map(t => 
            t.id === tx.id ? { ...t, status: newStatus, reason: note } : t
        ));
        ...
        await runTransaction(db, async (transaction) => {
            ...
            // 2. Status Update
            transaction.update(txRef, { status: newStatus, reason: note });
        });
  ...
  const handleSendAlert = async (e: React.FormEvent) => {
      ...
      if(!window.confirm(`Are you sure you want to broadcast this alert to ALL users?\n\nTitle: ${alertTitle}`)) return;

      setLoadingId('send-alert');
      try {
          // Deactivate all previous alerts first to avoid clutter
          const active = alerts.filter(a => a.active);
          for (const a of active) {
              await updateDoc(doc(db, "system_alerts", a.id), { active: false });
          }

          const newAlert: Omit<SystemAlert, 'id'> = {
              title: alertTitle,
          ...
          await addDoc(collection(db, "system_alerts"), newAlert);
  ...
  const handleToggleAlert = async (alertItem: SystemAlert) => {
      setLoadingId(alertItem.id);
      try {
          await updateDoc(doc(db, "system_alerts", alertItem.id), {
  ...
  const handleDeleteAllAlerts = async () => {
    if(!window.confirm("Are you sure you want to delete ALL alert history?")) return;
    setLoadingId('clear-alerts');
    try {
        const promises = alerts.map(a => deleteDoc(doc(db, "system_alerts", a.id)));
  ...
  // Derived State with Sorting & Filtering
  ...
  const allFilteredTransactions = getSortedTransactions(transactions.filter(tx => {
      ...
      // Search Filter (User ID, Email, Phone, TRX)
      if (searchQuery) {
          const q = searchQuery.toLowerCase();
          const emailMatch = (tx.email || '').toLowerCase().includes(q);
          const uidMatch = tx.userId.toLowerCase().includes(q);
          const trxMatch = (tx.trxId || '').toLowerCase().includes(q);
          const phoneMatch = (tx.senderNumber || tx.receiverNumber || '').includes(q);
          
          if (!emailMatch && !uidMatch && !trxMatch && !phoneMatch) return false;
      }
      ...
      if (dateFilter.start) {
          ...
          if (tx.timestamp < startDate.getTime()) return false;
      }
      if (dateFilter.end) {
          const endDate = new Date(dateFilter.end);
          endDate.setHours(23,59,59,999);
          if (tx.timestamp > endDate.getTime()) return false;
      }
    ...
    <div className="h-full w-full flex flex-col bg-[#0f172a] text-white overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col p-4 md:p-6 min-h-0">
                        ...
                        <form onSubmit={handleSendAlert} className="space-y-4">
                            <div className="space-y-1">
                                ...
                                <input 
                                    ...
                                    placeholder="e.g. System Maintenance, Bonus Event"
                                    required
                                    className="w-full bg-[#0f1522] border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 outline-none font-bold"
                                />
                            </div>
                            ...
                            <div className="space-y-1">
                                ...
                                <textarea 
                                    ...
                                    required
                                    rows={5}
                                    className="w-full bg-[#0f1522] border border-slate-700 rounded-xl p-3 text-white focus:border-blue-500 outline-none resize-none"
                                />
                            </div>

                            <div className="flex items-center gap-2 p-3 rounded-xl bg-[#0f1522] border border-slate-700 cursor-pointer" onClick={() => setIsUrgent(!isUrgent)}>
                                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isUrgent ? 'bg-red-500 border-red-500' : 'border-slate-500'}`}>
                                    {isUrgent && <CheckCircle className="w-4 h-4 text-white" />}
                                </div>
                                <span className={`text-sm font-bold ${isUrgent ? 'text-red-400' : 'text-slate-400'}`}>Urgent Alert</span>
                            </div>

                            <button 
                                type="submit"
                                disabled={loadingId === 'send-alert'}
                            ...
                            >
                                {loadingId === 'send-alert' ? 'Sending...' : <><Megaphone className="w-4 h-4" /> Broadcast Now</>}
                            </button>
                        </form>
                    </div>

                    {/* Alert History */}
                    <div className="bg-[#1e293b] rounded-2xl border border-slate-700 p-6 shadow-xl overflow-hidden flex flex-col">
                            ...
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                             ...
                             {alerts.length > 0 && (
                                <button 
                                    ...
                                    disabled={loadingId === 'clear-alerts'}
                                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 text-[10px] font-black uppercase tracking-wider transition-colors"
                                >
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
                            {alerts.length === 0 && <p className="text-slate-500 text-sm">No alerts history.</p>}
                            
                            {alerts.map(alert => (
                                <div key={alert.id} className={`p-4 rounded-xl border flex flex-col gap-2 transition-all ${alert.active ? 'bg-blue-900/10 border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'bg-[#0f1522] border-slate-700/50 opacity-70'}`}>
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-2">
                                            {alert.type === 'urgent' ? <AlertTriangle className="w-4 h-4 text-red-500" /> : <Bell className="w-4 h-4 text-blue-500" />}
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button 
                                                ...
                                                className={`p-2 rounded-lg transition-colors ${alert.active ? 'text-lime-400 bg-lime-500/10' : 'text-slate-600 bg-slate-800'}`}
                                                title={alert.active ? "Deactivate Alert" : "Activate Alert"}
                                            >
                                                {loadingId === alert.id ? <Loader2 className="w-6 h-6 animate-spin" /> : (alert.active ? <ToggleRight className="w-6 h-6" /> : <ToggleLeft className="w-6 h-6" />)}
                                            </button>
                                            <button 
                                    ...
                                    <span className="text-[9px] font-mono text-slate-600">{new Date(alert.timestamp).toLocaleString()}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                                ...
                                <tr key={tx.id} className="hover:bg-[#252f45] transition-colors group">
                                   ...
                                   <td className="p-4 text-xs text-slate-400">
                                      <div className="flex flex-col gap-0.5">
                                         ...
                                         {tx.reason && <span className="font-bold text-amber-500 text-xs mt-1 border-l-2 border-amber-500 pl-1">{tx.reason}</span>}
                                      </div>
                                   </td>
        ...
        {activeTab === 'all' && (
            <div className="flex-1 flex flex-col bg-[#1e293b] rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
                 {/* Filter Bar */}
                 <div className="p-4 bg-[#1a1d29] border-b border-slate-700 flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        <div className="flex-1 min-w-[200px] relative">
                            ...
                            <input 
                                type="text" 
                                placeholder="Search User, Email, TRX ID..." 
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="w-full bg-[#0f1522] border border-slate-700 rounded-xl py-2 pl-10 pr-4 text-xs font-bold text-white focus:border-blue-500 outline-none placeholder:text-slate-600"
                            />
                        </div>

                        {/* Date Range */}
                        <div className="flex items-center gap-2 bg-[#0f1522] p-1 rounded-xl border border-slate-700">
                            <div className="relative">
                                <input 
                                    type="date" 
                                    value={dateFilter.start}
                                    onChange={(e) => setDateFilter(prev => ({ ...prev, start: e.target.value }))}
                                    className="bg-transparent text-xs font-bold text-white border-none outline-none p-1 w-28 [color-scheme:dark]"
                                />
                            </div>
                         ...
                         {/* Reset */}
                         {(searchQuery || dateFilter.start || dateFilter.end || typeFilter !== 'all' || statusFilter !== 'all') && (
                             <button 
                                onClick={() => {
                                    setSearchQuery('');
                                ...
                                className="px-3 py-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[10px] font-bold uppercase transition-colors"
                             >
                                 Reset Filters
                             </button>
                         )}
                            ...
                            allFilteredTransactions.map(tx => (
                                <tr key={tx.id} className="hover:bg-[#252f45] transition-colors group">
                                   ...
                                   <td className="p-4 text-xs text-slate-300">
                                      <div className="font-bold text-white">{tx.email || 'Unknown'}</div>
                                      <div className="font-mono text-[9px] text-slate-500 opacity-50 group-hover:opacity-100 transition-opacity">{tx.userId}</div>
                                      {tx.reason && <span className="font-bold text-amber-500 text-xs block mt-1">{tx.reason}</span>}
                                   </td>
                                   <td className="p-4 text-right font-mono text-sm font-bold text-white">
                                      ${tx.amount.toLocaleString()}
                                   </td>
