...
import { CheckCircle, XCircle, Clock, Users, Wallet, Trash2, Archive, List, ArrowUpDown, ArrowUp, ArrowDown, Edit2, ChevronDown, PlusCircle, MinusCircle, Save, Lock, Unlock, Settings, ToggleLeft, ToggleRight, Smartphone, Banknote, AlertTriangle, Bell, Send, Megaphone } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const handleSendAlert = async (e: React.FormEvent) => {
      ...
      if(!window.confirm("Broadcast this alert to ALL users?")) return;
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button 
                                                ...
                                                className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-wider border transition-colors
                                                    ${alert.active ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-slate-700/50 text-slate-400 border-slate-600'}
                                                `}
                                            >
                                ...
                                <tr key={tx.id} className="hover:bg-[#252f45] transition-colors group">
                                   ...
                                   <td className="p-4 text-xs text-slate-400">
                                      <div className="flex flex-col gap-0.5">
                                         ...
                                         {tx.reason && <span className="font-bold text-amber-500 text-[10px] mt-1 border-l-2 border-amber-500 pl-1">{tx.reason}</span>}
                                      </div>
                                   </td>
                            ...
                            allFilteredTransactions.map(tx => (
                                <tr key={tx.id} className="hover:bg-[#252f45] transition-colors group">
                                   ...
                                   <td className="p-4 text-xs text-slate-300">
                                      <div className="font-bold text-white">{tx.email || 'Unknown'}</div>
                                      <div className="font-mono text-[9px] text-slate-500 opacity-50 group-hover:opacity-100 transition-opacity">{tx.userId}</div>
                                      {tx.reason && <span className="font-bold text-amber-500 text-[10px] block mt-1">{tx.reason}</span>}
                                   </td>
                                   <td className="p-4 text-right font-mono text-sm font-bold text-white">
                                      ${tx.amount.toLocaleString()}
                                   </td>
