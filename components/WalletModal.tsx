...
import { X, Copy, History, ArrowDownCircle, ArrowUpCircle, CheckCircle, Clock, XCircle, AlertCircle, Lock, Shield, Loader2 } from 'lucide-react';
...
const WalletModal: React.FC<WalletModalProps> = ({ isOpen, onClose, userId, userEmail, balance, gameMode, isAdmin = false }) => {
  ...
  useEffect(() => {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          // Set default method if available and none selected
    ...
    try {
        ...
        if (activeTab === 'deposit') {
            ...
            if (cleanTrx.length < 6) {
                throw new Error("Invalid Transaction ID. It seems too short.");
            }
        ...
        <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
            {activeTab === 'history' ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    {/* Admin Number Display (Deposit Only) - DYNAMIC */}
                    {activeTab === 'deposit' && getActiveMethodDetails() && (
                        <div className="bg-[#0f1522] p-4 rounded-xl border border-slate-700 text-center relative overflow-hidden">
                            ...
                            <p className="text-[10px] uppercase text-slate-400 font-bold mb-1">
                                Send Money to this {getActiveMethodDetails()?.name} Number
                            </p>
                            ...
                            <p className="text-[9px] text-slate-500 mt-1 bg-slate-800/50 inline-block px-2 py-1 rounded">
                                Use "Send Money" or "Cash Out" as per agent instruction.
                            </p>
                        </div>
                    )}
