...
const App: React.FC = () => {
  ...
  const [autoBetRemaining, setAutoBetRemaining] = useState<number>(0); // 0 = infinite internally for calculation, or huge number
  ...
  const handlePlay = useCallback(async (): Promise<boolean> => {
    ...
    if (user) {
        ...
        try {
            ...
            // Revert optimistic update if needed, but Firestore listener usually corrects this quickly
            return false;
        }
    }
    return false;
  ...
  // --- Auto Bet Logic ---
  const handleAutoStart = (count: number) => {
      // count 0 means infinite. We represent infinite as -1 internally for logic, or just a very high number? 
      // Let's use -1 for infinite.
      setAutoBetRemaining(count === 0 ? -1 : count);
  ...
  useEffect(() => {
      ...
      if (isAutoBetting) {
          const runAutoLoop = async () => {
              // 1. Check Balance STRICTLY
              // If balance is zero, negative, or less than bet amount, STOP immediately.
              if (currentBalance <= 0 || currentBalance < betAmount) {
                  setIsAutoBetting(false);
                  return;
              }

              // 2. Check Count (if not infinite)
              ...
              // 4. Decrement Count
              if (autoBetRemaining !== -1) {
                  setAutoBetRemaining(prev => prev - 1);
              }

              // 5. Schedule Next
              // 200ms delay for rapid fire
              timeoutId = setTimeout(runAutoLoop, 200);
          };
  ...
  }, [isAutoBetting, autoBetRemaining, currentBalance, betAmount, handlePlay]);
    ...
    <div className="h-[100dvh] w-screen bg-[#1a1d29] text-white flex flex-col font-sans overflow-hidden relative selection:bg-lime-500/30 pt-safe">
      ...
      <nav className="h-14 sm:h-16 bg-[#1a1d29]/90 backdrop-blur-md flex items-center justify-between px-3 md:px-6 border-b border-slate-700/50 shadow-lg relative z-50 shrink-0">
         <div className="flex items-center gap-2 md:gap-4">
             ...
             {/* Mobile: History Toggle (Hidden on LG+) */}
             <button 
      ...
      <div className="relative z-10 flex-1 flex w-full h-[calc(100dvh-56px)] overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#0f1522]/30 relative">
            {/* Game Board Area - Optimized for larger screens */}
