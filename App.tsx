...
import { ShieldCheck, LogOut, Wallet, RotateCcw, Shield, History as HistoryIcon, X, Menu, CreditCard, Copy, MessageCircle, Globe } from 'lucide-react';
...
// Define a serializable user type to avoid Circular JSON errors
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser: any) => {
      if (currentUser) {
         ...
         // Store only serializable properties to avoid Circular Structure errors
         setUser({
             uid: currentUser.uid,
  ...
  if (!user) {
      return (
        <>
            <NetworkMonitor />
            <LoginPage currentUser={null} lang={lang} setLang={setLang} />
        </>
      );
  }
